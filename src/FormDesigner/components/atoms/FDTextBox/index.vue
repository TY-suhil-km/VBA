...
export default class FDTextBox extends Mixins(FdControlVue) {
  ...
  finalStart: boolean = false
  finalEnd: boolean = false
  dblclick (e: Event) {
   ...
   */
  onTextSelectionChange (event: Event) {
    event.preventDefault()
    const eventArea = this.textareaRef
    let start = eventArea.selectionStart
    let end = eventArea.selectionEnd
    const format = /[`!@#$%^&*()_+\-=[\]{};':"\\|,.<>/?~]/
    if (start !== end) {
      if (eventArea.value[end - 1] !== ' ' && eventArea.value[end - 2] === ' ' && !this.finalStart) {
        for (let i = start; i >= 0; i--) {
          if (eventArea.value[i - 1] === ' ' || eventArea.value[i - 1] === undefined || eventArea.value[i - 1].match(format)) {
            start = i
      ...
      if (this.finalStart) {
        for (let j = end; j <= eventArea.value.length; j++) {
          end = j
          if ((eventArea.value[j - 1] === ' ' && eventArea.value[j] !== ' ') || eventArea.value[j] === undefined || eventArea.value[j].match(format)) {
            eventArea.setSelectionRange(start, j)
            break
          }
        }
      } else if (eventArea.value[start] === ' ' && eventArea.value[end - 1] !== ' ' && eventArea.value[start - 1] !== ' ' && !this.finalStart) {
        for (let i = end + 1; i <= eventArea.value.length; i++) {
          if (((eventArea.value[i - 1] === ' ' && eventArea.value[i] !== ' ') || eventArea.value[i] === undefined || eventArea.value[i].match(format)) && !this.finalEnd) {
            end = i
            this.finalEnd = true
            break
          }
        ...
        for (let j = start; j >= 0; j--) {
          if (eventArea.value[j - 1] === ' ' || eventArea.value[j - 1] === undefined || eventArea.value[j - 1].match(format)) {
            start = j
            eventArea.setSelectionRange(start, end, 'backward')
            break
          }
        }
      }
    } else if (start === end) {
      this.finalStart = false
      this.finalEnd = false
    }
  }
  /**
